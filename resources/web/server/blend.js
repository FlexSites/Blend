...
const SERVICE_ACCOUNT_KEY_FILE = path.join(__dirname, 'elevated-numbers.pem')
...
module.exports.getBlend = function (selectedOptions, selectedYears, sepTXPOC, sepColor, callback) {
  ...
  var getReq = function (query, option, callback) {
    drive.children.list({
      auth: jwt,
      q: query,
      folderId: option.id,
  ...
  Bluebird.reduce(
    selectedOptions, 
    (all, option) => listFiles(option.id).then(({ items }) => all.concat(items)),
    []
  )
